...
struct ConcurrentScalableCache {
   ...
   *     count).
   */
  explicit ConcurrentScalableCache(size_t maxSize, int64_t ttlSecond,
                                   size_t numShards = 0);
...
ConcurrentScalableCache<TKey, TValue, THash>::ConcurrentScalableCache(
    size_t maxSize, int64_t ttlSecond, size_t numShards)
...
template <class TKey, class TValue, class THash>
bool ConcurrentScalableCache<TKey, TValue, THash>::find(ConstAccessor &ac,
                                                        const TKey &key) {
...
template <class TKey, class TValue, class THash>
bool ConcurrentScalableCache<TKey, TValue, THash>::warmup(const TKey &key,
                                                          const TValue &value,
                                                          uint64_t extra_sec) {
  return getShard(key).warmup(key, value, extra_sec);
}

template <class TKey, class TValue, class THash>
void ConcurrentScalableCache<TKey, TValue, THash>::clear() {
  for (size_t i = 0; i < m_numShards; i++) {
    m_shards[i]->clear();
  }
}
...
template <class TKey, class TValue, class THash>
size_t ConcurrentScalableCache<TKey, TValue, THash>::size() const {
  size_t size = 0;
  for (size_t i = 0; i < m_numShards; i++) {
